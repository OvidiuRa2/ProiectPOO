...
//Validare ca un string este format din litere si spatii
...
//Validare ca un string este format numai din cifre
 ...
 bool MetodeUtile::valideazaTipFisier(string numeFisier) {
	 string end = ".txt";
	
	int position = numeFisier.rfind(end);
	
	  if (position + end.length() == numeFisier.length()) {
		  return true;
	  }
	  else {
		  return false;
 ...
 bool MetodeUtile::verificarePrimaDataEsteInainte(string data1, string data2) {
	 ...
	 int ziData2 = stoi(data2.substr(0, 2));
	 int lunaData2 = stoi(data2.substr(3, 2));
	 int anData2 = stoi(data2.substr(6, 4));
	 
	 int ziData1 = stoi(data1.substr(0, 2));
	 int lunaData1 = stoi(data1.substr(3, 2));
	 int anData1 = stoi(data1.substr(6, 4));
 ...
 bool MetodeUtile::verificarePrimaOraEsteInainte(string ora1, string ora2) {
	 int oraData1 =stoi( ora1.substr(0, 2));
	 int minutData1 =stoi( ora1.substr(3, 2));

	 int oraData2 =stoi( ora2.substr(0, 2));
	 int minutData2 = stoi(ora2.substr(3, 2));

	 if (oraData2 > oraData1) {
		 return true;
	 }else if(oraData2==oraData1) {
		 if (minutData2 > minutData1) {
			 return true;
		 }
		 else {
			 return false;
 ...
 bool MetodeUtile::valideazaOra(string s) {
  ...
  string MetodeUtile::toLowerCase(string s) {
	  string result="";
