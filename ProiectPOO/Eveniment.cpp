...
list<Eveniment>* Eveniment::evenimenteCreate = new list<Eveniment>();
...
Eveniment::Eveniment() {
	...
	evenimenteCreate->push_back(*this);
}
...
list<Eveniment> Eveniment::getEvenimenteCreate() {
	return *evenimenteCreate;
}
...
//validare ca locul pentru care vreau sa creez bilet nu a mai fost deja alocat
...
void Eveniment::afiseazaEvenimenteCreate() {
	list<Eveniment>::iterator it;
	for (it = evenimenteCreate->begin(); it != evenimenteCreate->end(); ++it)
		cout << *it << endl;
}
...
Eveniment& Eveniment::contineEveniment(string denumireEveniment) {
	list<Eveniment>::iterator it;
	for (it = evenimenteCreate->begin(); it != evenimenteCreate->end(); ++it)
	{
		if (it->getDenumire() == denumireEveniment) {
			return *it;
		}
	}
	throw  exception("Evenimentul introdus nu exista");
}

bool Eveniment::contineEveniment(string denumireEveniment,bool) {
	list<Eveniment>::iterator it;
	for (it = evenimenteCreate->begin(); it != evenimenteCreate->end(); ++it)
	{
		if (it->getDenumire() == denumireEveniment) {
...
bool  Eveniment::operator<(Eveniment e) {
	if (data != "" && e.data != "") {
	//verificare si dupa ora in caz ca sunt egale
		if (data == e.data) {
			return MetodeUtile::verificarePrimaOraEsteInainte(ora,e.ora);
		}
		else {
